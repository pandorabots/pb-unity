/*********************************************************************
* This code has been modified from its original file "ExampleStreaming.cs"
* by Hourlab LLC to suit the needs of Virtual Reality Devices.
* Below is the original copyright provided by IBM...
* 
...
{
    public class WatsonStreamingSTT : MonoBehaviour
    {
        ...
        [Tooltip("Text field to tell user how to restart service.")]
        public TextMeshProUGUI botMessage;
        ...
        [SerializeField]
        private StringEvent onWatsonSTTResponse;
        #endregion
        ...
        private void Update()
        {
            // If IBM Watson Streaming Service Disconnected
            if (OVRInput.GetDown(OVRInput.RawButton.B))
            {
                _service.StartListening(OnRecognize, OnRecognizeSpeaker);
                botMessage.text = "Bot is now listening...";
                Debug.Log("service manually started");
            }
            if (_service != null)
            {
                if (!_service.IsListening)
                {
                    botMessage.text = "Bot is no longer listening, please press \"B\" to continue conversation";
                }
            }
            #region If not using headphones/headphones pick up mic audio
            /*
            ...
            if (Input.GetKeyDown(KeyCode.Escape))
            {
                ...
                Debug.Log("service manually stopped");
            }
            if (_service != null)
            {
                if (!_service.IsListening)
                ...
                else
                {
                    if (botAudioSource.isPlaying)
                    {
                        Debug.Log("stop listening while playing");
                        _service.StopListening();
                    }
                }
            }
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if (res.final)
                        {
                            onWatsonSTTResponse.Raise(text);
                        }
                    }
